...
class StartTask(override val plugin: KVaroPlugin, private var counter: Int = 60): Runnable, WithPlugin<KVaroPlugin> {
    ...
    private val claims = listOf("%s hat den Start verpennt", "%s wird als erstes sterben", "%s wird den Stegi machen",
            "%s versteht 1.9er PvP nicht", "%s überlebt die 2. Nacht nicht", "fretoger kann kein Mathe",
            "%s ist vom Himmel gefallen", "SheKlotzLP ist ein 5. Österreicher", "%s wird gewinnen", "MinecraftiUndCo ist \"kurz\" afk",
            "%s timed out", "%s findet niemals Dias", "%s wird in der Hölle verglühen")
    ...
    private fun getRandomPlayer(): String {
        return Bukkit.getOnlinePlayers().map { it.displayName }.random()
    }
